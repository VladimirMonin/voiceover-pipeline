...
> недокументированное поле `multi_speaker_voice_config` игнорируется, и
> live-прослушивание подтвердило один женский голос для обоих спикеров
> (live-приёмка FAILED/BLOCKED:
> `docs/reports/2026-08-21-gemini-dialogue-live-acceptance.md`). Пока не
> внедрён план фикса
> (`docs/plans/2026-08-22-agent-first-twovoice-dialogue-fix-plan.md`):
> - НЕ заявляй, что двухголосый результат работает, и НЕ отправляй гибридный
>   payload с `multi_speaker_voice_config`.
> - Если пользователю нужен гарантированный двухголосый результат сейчас —
>   скажи, что эта возможность перерабатывается, и предложи альтернативу
>   (например, отдельные одноголосые прогоны).
> - Когда фикс внедрён, поведение — по плану: **один запрос на реплику
>   (turn) с одним документированным top-level `voice`**, реплики склеиваются
>   с детерминированными паузами; `Alias:` не попадает в озвученный текст.

...
> **Совместимость:** voiceover-pipeline 0.4.0, skill revision 2026-04-29.
...
1. **Безопасность прежде всего.** Прочитай `docs/03-security-and-secrets.md`.
   Создай `.env.example`, убедись в `.gitignore`, попроси ключи ОДИН раз.
...
- Попроси пользователя ОДИН раз скопировать в `.env` и вписать ключи.

...
- Python implementation and independent-review cards inherit the quality gates from `instructions/test-quality.instructions.md`: Ruff lint, Ruff formatting check, and mypy must be run on the exact handed-off bytes. Ordinary one-to-three-line lint/format/type residue is fixed in the current card and verified directly, not split into a new card or used to rerun a completed implementation.
